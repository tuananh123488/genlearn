...
import { GrammarInterface } from '../context/interfaces'
import { ThemeContext } from '../context/themeContext'
import { StatusToast } from '../toast'
import { shuffleArray } from '@/utils/vocabulary/vocabulary'

export interface OptionsProp {
    results: GrammarInterface[]
    setResults: React.Dispatch<React.SetStateAction<GrammarInterface[]>>
    setGrammarPractice: React.Dispatch<React.SetStateAction<GrammarInterface[]>>
    setLanguages: React.Dispatch<React.SetStateAction<string>>
}

const Options = ({ results, setResults, setGrammarPractice, setLanguages }: OptionsProp) => {
    ...
    const handleStartTest = () => {
        const { type, numberOfVocabularies, from, to, languages } = options
        if (type === '') {
    ...
    return (
        <section className='w-[30%] flex flex-col items-center'>
            ...
            <h1 className='text-[1.5rem] font-poppins font-semibold mb-[2rem]'>Practice Grammar</h1>
            <Select
